...
class Editor {
    ...
    streamParser = {
        ...
        token: (stream, state) => {
            if ( stream.sol() ) {
                if ( stream.match(/^---\s*$/) ) { return 'adblock-boundary'; }
                if ( stream.match(/^# ---\s*$/) ) { return 'adblock-boundary adblock-comment'; }
                if ( stream.match(/\.\.\.\s*$/) ) { return 'adblock-boundary'; }
            }
            const c = stream.peek();
            if ( c === '#' ) {
            ...
            if ( scope === 0 && stream.match(/^[^:]+(?=:)/) ) {
                state.scope = 1;
                return 'adblock-keyword';
            }
            if ( scope === 1 && stream.match(/^:(?: |$)/) ) {
                return 'adblock-punctuation';
            }
            if ( stream.match(/^- /) ) {
                return 'adblock-punctuation';
            }
