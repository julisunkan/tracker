...
async function reportSpecificFilterIssue() {
    ...
    reportURL.searchParams.set('category', issueType);
    ...
    reportURL.searchParams.set('configuration', configBody);
    sendMessage({ what: 'gotoURL', url: reportURL.href });
}
...
getTroubleshootingInfo(reportedPage).then(config => {
    ...
    if ( reportedPage !== null ) {
        ...
        dom.on('[data-i18n="supportFindSpecificButton"]', 'click', ev => {
            const url = new URL('https://adblockall.com/issues');
