...
function jsonlEditFetchResponseFn(trusted, jsonq = '') {
    ...
    if ( jsonp.valid === false || jsonp.value !== undefined && trusted !== true ) {
        return safe.AdBlockAllog(logPrefix, 'Bad JSONPath query');
    }
    ...
    proxyApplyFn('fetch', function(context) {
        ...
        if ( propNeedles.size !== 0 ) {
            ...
            if ( safe.logLevel > 1 ) {
                safe.AdBlockAllog(logPrefix, `Matched "propsToMatch":\n\t${matched.join('\n\t')}`);
            }
        }
        return fetchPromise.then(responseBefore => {
            ...
            return response.text().then(textBefore => {
                ...
                if ( logall ) {
                    safe.AdBlockAllog(logPrefix, textBefore);
                    return responseBefore;
                }
                const textAfter = jsonlEditFn(jsonp, textBefore);
                if ( textAfter === textBefore ) { return responseBefore; }
                safe.AdBlockAllog(logPrefix, 'Pruned');
...
function preventSetTimeout(
    ...
    proxyApplyFn('setTimeout', function(context) {
        ...
        if ( needleRaw === '' && range.unbound() ) {
            safe.AdBlockAllog(logPrefix, `Called:\n${a}\n${b}`);
            return context.reflect();
        }
        if ( reNeedle.test(a) !== needleNot && range.test(b) ) {
            callArgs[0] = function(){};
            safe.AdBlockAllog(logPrefix, `Prevented:\n${a}\n${b}`);
        }
        return context.reflect();
    });
}
...
function setConstantFn(
    ...
    function setConstant(chain, rawValue) {
        ...
        const mustAbort = function(v) {
            ...
            if ( aborted ) {
                safe.AdBlockAllog(logPrefix, `Aborted because value set to ${v}`);
            }
            return aborted;
        };
        ...
        const trapProp = function(owner, prop, configurable, handler) {
            ...
            try {
                ...
                safe.AdBlockAllog(logPrefix, 'Trap installed');
            } catch(ex) {
            ...
            if ( pos === -1 ) {
                trapProp(owner, chain, false, {
                    ...
                    getter: function() {
                        ...
                        safe.AdBlockAllog(logPrefix, 'Property read');
                        return normalValue;
                    },
                    setter: function(a) {
    ...
    proxyApplyFn(methodPath, function(context) {
        ...
        for ( const elem of elems ) {
            try {
                ...
                safe.AdBlockAllog(logPrefix, 'Bypass prevented');
            } catch {
            }
        }
        return r;
...
const $scriptletHostnames$ = /* 4 */ ["localhost","localhost>>","AdBlockAll.github.io","AdBlockAll.github.io>>"];
